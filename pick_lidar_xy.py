#!/usr/bin/env python3
"""实时显示相机图像，点击像素后返回最近的雷达点坐标。

默认订阅：
- 相机图像: /camera/camera/color/image_raw
- 相机参数: /camera/camera/color/camera_info
- 雷达点云: /livox/lidar

说明：
- 点击的是图像像素，程序会在当前点云投影结果里找最近的雷达点。
- 如果相机和雷达没有严格同步，点击结果会有一定误差。
- 默认使用标定结果文件 calib_data/results/extrinsic.txt 里的外参矩阵。
"""
...
class PixelToLidarPicker(Node):
    def __init__(self, args: argparse.Namespace):
        super().__init__("pixel_to_lidar_picker")
        ...
        if args.fx > 0 and args.fy > 0 and args.cx >= 0 and args.cy >= 0:
            ...
            # Try to subscribe to camera_info, but also use defaults if it doesn't arrive
            ...
            # Set a fallback timer to use default intrinsics if camera_info doesn't arrive
    ...
    def camera_info_callback(self, msg: CameraInfo) -> None:
        ...
        self.get_logger().info("已接收到相机参数")
        # Cancel fallback timer since we got camera_info
        if hasattr(self, 'fallback_timer'):
            self.destroy_timer(self.fallback_timer)

    def _use_fallback_intrinsics(self) -> None:
        """使用 RealSense D435i 的默认内参，如果 camera_info 没有在 5 秒内到达。"""
        if self.camera_matrix is not None and self.dist_coeffs is not None:
            return  # 已经收到 camera_info
        
        # RealSense D435i default intrinsics for 1280x720 resolution
        fx, fy = 918.3, 917.6
        cx, cy = 640.0, 359.6
        self.camera_matrix = np.array(
        ...
        self.dist_coeffs = np.zeros((1, 5), dtype=np.float64)
        self.get_logger().info(f"相机信息没有按时到达，使用 RealSense D435i 默认内参: fx={fx}, fy={fy}, cx={cx}, cy={cy}")
        self.need_reproject = True
        # Destroy the fallback timer
        if hasattr(self, 'fallback_timer'):
            self.destroy_timer(self.fallback_timer)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="实时显示图像并点击查询雷达坐标")
    parser.add_argument(
        "--image-topic",
    ...
    parser.add_argument("--fx", type=float, default=0.0, help="相机 fx，提供后可不订阅 camera_info")
    parser.add_argument("--fy", type=float, default=0.0, help="相机 fy，提供后可不订阅 camera_info")
    parser.add_argument("--cx", type=float, default=-1.0, help="相机 cx，提供后可不订阅 camera_info")
    parser.add_argument("--cy", type=float, default=-1.0, help="相机 cy，提供后可不订阅 camera_info")
    parser.add_argument(
        "--window-name",
        default="LiDAR Image Picker",
        help="OpenCV 窗口名称",
    )
    return parser
